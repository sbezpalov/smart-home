...
|-----|--------------------|------|--------|------------|
| Датчики дыма | *(модель — уточнить, контур — уточнить)* | Все жилые комнаты + Bathhouse | *(Aqara? Hikvision? автономные?)* | Пожарная сигнализация |
| Датчик газа | *(модель — уточнить)* | Boiler Room | *(контур — уточнить)* | Утечка газа |
...
- **Котёл Kiturami на WB-Zigbee** — статус: planned или уже подключён? Это меняет SLA-класс WB с «промышленный контроллер» на «критическая инженерная система», что влечёт: бэкап конфигурации WB (ADR-002), мониторинг в Zabbix, runbook на ручное управление котлом при отказе WB.
